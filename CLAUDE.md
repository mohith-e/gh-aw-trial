...
| `hello-self-hosted` | Manual | Minimal self-hosted runner example. Pins `enterprise-global-runners` and verifies an internal-only host (`artifacts.realpage.com`) is reachable via TLS handshake — reaching Anthropic alone would not prove internal reachability. Deliberately thin so adopters don't cargo-cult diagnostics into every workflow |
| `netcheck-self-hosted` | Manual | Diagnostic companion to `hello-self-hosted`. Plain GitHub Actions (no AWF/WIF/engine) — per host, resolves DNS, probes TCP/443, and runs three TLS handshakes to distinguish SNI blocking from routing from DNS. Like `tfs-mirror.yml`, it is plain `.yml` that `gh aw add` does not distribute; consumers copy it manually |
| `tfs-implement` | Every 15 min off-hour / manual with work item ID | Implements one Azure DevOps (TFS) work item per run — claims via tag state machine, clones the target branch, writes code in an `agent/wi-*` branch, pushes back to TFS, and opens a PR in TFS with PR review notes. Reusable across teams; configure via repo variables |
| `tfs-review-pr` | Every 15 min off-hour / manual with PR ID / `/review-ai` comment | AI review of TFS pull requests. A coordinator run scans TFS and fans out up to `TFS_REVIEW_BATCH_SIZE` worker runs, each reviewing one PR and posting a summary thread plus up to 8 inline findings back onto the TFS PR. Advisory only — it never votes on or blocks the PR |
...
- **Repo-variable naming.** `vars.*` and `secrets.*` are a single repo-global namespace shared by every workflow a consumer installs together as a suite. A variable that is *genuinely* shared config for a family of workflows keeps a bare name (e.g. `TFS_BASE`, `TFS_REPO`, `TFS_USE_MIRROR`, `TFS_TARGET_BRANCH`, `TFS_PROJECT`, `TFS_TEAM_AREA_PATH`, `TFS_PAT` — read identically by all the `tfs-*` workflows). A variable specific to one workflow MUST carry that workflow's prefix (`TFS_MIRROR_*` for `tfs-mirror`, `TFS_REVIEW_*` for `tfs-review-pr`, e.g. `TFS_MIRROR_BOOTSTRAP_BRANCH`, `TFS_REVIEW_MAX_AGE_DAYS`) — never reuse a bare/shared name for a different purpose. Reusing a shared name causes a silent collision when both workflows are installed in the same repo: they read the same value, so one workflow's setting corrupts the other's behavior with no error. (Purely internal values that never leave the workflow file — job/step `env:` used only inside `run:` blocks — are not in the consumer namespace and don't need a prefix.)
...
- The `tfs-review-pr` workflow requires variables `TFS_BASE`, `TFS_REPO` plus secret `TFS_REVIEW_PAT` (falling back to `TFS_PAT` when unset — a narrower Code: Read + Pull Request Threads scope is recommended, since it never pushes). Optional `TFS_REVIEW_SCHEDULE_ENABLED`, `TFS_REVIEW_MAX_AGE_DAYS`, `TFS_REVIEW_BATCH_SIZE`. It deliberately does NOT read `TFS_TARGET_BRANCH` — it reviews PRs against every target branch, and reusing that name would silently narrow reviews to the implementer's one branch.
- **Clone transport (`TFS_USE_MIRROR`).** Both TFS workflows need a working tree of the target branch and get one either straight from TFS or by cloning the GitHub mirror ref `tfs-mirror/<target-branch>` and fetching only the delta from TFS. Unset probes for the mirror ref and uses it when present; `true` also warns when it is absent; `false` never probes. The mirror requires the companion `workflows/tfs-mirror.yml` — plain GHA YAML that `gh aw add` does not distribute; consumers copy it into `.github/workflows/` manually. It is worth installing when a direct TFS clone dominates run time on a large repo. The mirror is transport only: `base_sha` and all ancestry checks reference the freshly fetched TFS tip, and mirrored SHAs are byte-identical to TFS (force-push, one-way, never merged). GitHub `main` in the consumer repo holds only workflow files; recommend a ruleset blocking human pushes to `tfs-mirror/**`.
- **Shared TFS modules.** `workflows/shared/tfs/` holds the pieces both TFS workflows import: `core.md` (network allow-list and the workflow-level `env:` for `TFS_BASE` / `TFS_REPO` / `TFS_USE_MIRROR`), plus one file per safe-output handler job. gh-aw fails compilation when two imports define the same workflow-level `env:` key, so `core.md` is the only module carrying one — a TFS variable read by more than one workflow belongs there and nowhere else. The module graph is flat by design: no module imports another, which keeps path resolution simple at the consumer's `gh aw compile`. `gh aw add` fetches the whole subdirectory alongside `shared/wif-engine.md`.
