...
| `tfs-implement` | Every 15 min off-hour / manual with work item ID | Implements one Azure DevOps (TFS) work item per run — claims via tag state machine, clones via GitHub mirror + TFS delta fetch, writes code in an `agent/wi-*` branch, pushes back to TFS, and opens a PR in TFS with PR review notes. Reusable across teams; configure via repo variables. Pairs with `tfs-mirror.yml` for the optimized clone path |
| `tfs-mirror` (`tfs-mirror.yml`) | Every 10 min off-hour / manual | Companion to `tfs-implement`. Mirrors every TFS branch into GitHub under `tfs-mirror/<branch>` so the implementer can clone cheaply from GitHub and fetch only the TFS delta. Plain GHA YAML (not a gh-aw `.md`) — install by copying the file into `.github/workflows/`, no `gh aw compile` step |
...
- The `tfs-implement` workflow requires variables `TFS_BASE`, `TFS_PROJECT`, `TFS_TEAM_AREA_PATH`, `TFS_REPO`, `TFS_TARGET_BRANCH` plus secrets `TFS_PAT` and `ANTHROPIC_API_KEY` in the consumer repo. Defaults assume `tfs.realpage.com`; consumers on a different TFS / Azure DevOps host must override `network.allowed` in their stub. The workflow uses a three-phase architecture: a deterministic pre-agent step claims/clones (PAT scoped), the agent edits and emits a format-patch (no PAT), and safe-output handler jobs `tfs-finalize-pull-request` / `tfs-record-failure` mediate all TFS writes — so `strict: true` and the agent prompt has no TFS REST calls. The clone path probes for the GitHub `tfs-mirror/<target>` branch maintained by the companion `tfs-mirror.yml` workflow and fetches only the TFS delta when the mirror is present; falls back to a non-mirrored clone (no fetch optimization) when it is not — both paths produce identical results, the mirror is purely a transfer-size optimization.
- The `tfs-mirror.yml` companion workflow requires variables `TFS_BASE`, `TFS_REPO` and secret `TFS_PAT` (shares the same names as `tfs-implement` so consumers configure them once). It runs every 10 minutes on cron `3,13,23,33,43,53`, staggered against `tfs-implement`'s `7,22,37,52` schedule. Plain GHA YAML, not a gh-aw `.md`, so it is not compiled — but `tfs-implement.md` declares it in `resources:`, so `gh aw add` of the implementer fetches `tfs-mirror.yml` alongside it (same pinned ref) into `.github/workflows/`. Consumers no longer copy it manually.
