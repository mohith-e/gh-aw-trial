---
description: |
  TFS (Azure DevOps) work item implementer — MIRRORED variant for large repos.
  Same contract as tfs-implement (polls a team's work item queue, claims one
  item via a tag state machine, lets the agent write code and produce a
  format-patch without TFS credentials, and mediates all TFS writes through
  safe-output handler jobs), but both clone sites start from the GitHub
  mirror refs maintained by .github/workflows/tfs-mirror.yml
  (refs/heads/tfs-mirror/<branch>) and then fetch only the small delta from
  TFS. This bounds the per-run TFS transfer to minutes of commits instead of
  the whole repo — the fix for huge repos where a direct clone from TFS is
  the bottleneck. If the mirror ref is missing (bootstrap, or tfs-mirror.yml
  never ran), both sites fall back to a full TFS fetch: slow once, correct
  always. TFS remains authoritative — base_sha and all ancestry checks are
  computed against the freshly fetched TFS tip, never the mirror ref.
  Reusable across teams; configure per-team values via repo variables — see
  the verify step below for the authoritative list.

# ── Triggers ──────────────────────────────────────────────────────────────────
#
# This variant REPLACES tfs-implement for a team — adopt one or the other, not
# both. Running both schedules against the same queue is safe (the claim is
# atomic — one run wins, the other no-ops on 409) but wasteful and noisy; if
# you are migrating from tfs-implement, remove its schedule (or the whole
# stub) when you adopt this one.
#
# COMPANION WORKFLOW REQUIRED: workflows/tfs-mirror.yml from this repo must be
# copied into the consumer repo's .github/workflows/ — it is plain GitHub
# Actions YAML, which `gh aw add` does not distribute. Without it every run
# falls back to a full TFS fetch: correct, but as slow as plain tfs-implement.
#
# Off-the-hour minutes (7,22,37,52): GitHub Actions throttles workflows firing
# on common boundaries like :00, :10, :15 — and these are staggered against
# tfs-mirror.yml's 3,13,23,33,43,53 so the implementer usually runs a few
# minutes after a fresh mirror sync.
# ──────────────────────────────────────────────────────────────────────────────
on:
  schedule:
    - cron: "7,22,37,52 * * * *"
...
permissions:
  ...
  # requires it; the shorthand can't carry it). `contents: read` also covers
  # the GITHUB_TOKEN mirror clones in the claim step and finalize handler.
  contents: read
  id-token: write
...
# `defaults` already covers GitHub itself, which the mirror clone needs.
#
...
env:
  ...
  # The agent step has no access to the PAT. GITHUB_TOKEN follows the same
  # discipline: bound only to the two steps that clone the GitHub mirror,
  # never at workflow level. The local clone under $GITHUB_WORKSPACE/tfs-work
  # has BOTH credential headers stripped after clone, so the agent cannot
  # push to TFS or GitHub — the TFS push is mediated by the finalize handler,
  # which clones fresh in its own job and `git am`s the agent's format-patch
  # onto a controlled commit before pushing.
...
#
# After cloning, both local `http.extraheader` values are unset so the agent
# literally cannot push anywhere — push is mediated by the safe-output
# handler below, which gets the agent's commits via a format-patch in the
# workspace artifact.
steps:
  - name: Verify required secrets and variables
    ...
    run: |
      ...
      This variant also expects .github/workflows/tfs-mirror.yml to be
      installed and running — copy it from
      RealPage/agentic-workflows/workflows/tfs-mirror.yml. It maintains the
      refs/heads/tfs-mirror/* branches both clone sites start from. Without
      it every run falls back to a full TFS fetch (correct but slow).

      Note: the hostname in TFS_BASE must match the entry in `network.allowed`
      at the top of this workflow's .md source. If you point this workflow at
      a different TFS instance, update both.
      ─────────────────────────────────────────────────────────────────
      EOF
      exit 1
  ...
  - name: Claim work item, clone TFS, prepare branch
    ...
    run: |
      ...
      # Instead of cloning from TFS (slow for huge repos — the tip tree alone
      # can be gigabytes over the WAN), start from the GitHub mirror that
      ...
      # GitHub-side history is already close to current; the TFS fetch below
      # transfers at most a few minutes of commits.
      #
      # NOTE: the finalize handler uses the same mirror-clone pattern (search
      # "GitHub mirror clone" in its step) with its own token scopes. That
      # second clone is a deliberate security boundary, not duplication to
      # factor out — keep the auth handling consistent between the two.
      ...
      # The tip comes from the fresh TFS fetch (`tfs/...`), NEVER the mirror
      ...
      # remote. Both headers were passed one-shot via `-c` (git does NOT
      # persist a `-c` value into the new repo's config), but unset
      # defensively. Push to TFS is mediated by the safe-output handler that
      # has its own PAT scope.
...
safe-outputs:
  jobs:
    tfs-finalize-pull-request:
      ...
      # exists. We need `contents: read` so the GitHub mirror clone below
      # works; the TFS write goes via TFS_PAT, not this token, so read is
      # enough.
      permissions:
        contents: read
      env:
      ...
      steps:
        ...
        - name: Push branch to TFS, open PR, post PR review-notes thread, link WI, transition tag
          run: |
            ...
            # ---------- GitHub mirror clone (cheap) + incremental TFS fetch ----------
            # Same pattern as the claim step: start from the GitHub mirror
            # ref, then fetch the delta from TFS. Unlike the direct-clone
            # variant of this workflow (which needed FULL history from TFS to
            # guarantee base_sha is present), the mirror clone carries full
            # history already — the TFS fetch on top makes base_sha reachable
            # even if the target advanced mid-run (base_sha is always an
            # ancestor of the current tip; the branch only advances under
            # branch protection).
            GH_REPO_URL="https://github.com/${GITHUB_REPOSITORY}.git"
            MIRROR_BRANCH="tfs-mirror/${TFS_TARGET_BRANCH}"
            ...
            # clone, so nothing should be persisted; this is belt-and-braces).
...
# TFS Work Item Implementer (Mirrored)
...
The handler job will, in a separate job with the PAT scoped to its env: clone the GitHub mirror of the target branch, fetch the fresh delta from TFS, `git am` the patch you wrote to `/tmp/gh-aw/agent/aw-tfs-wi-<id>.patch` (which it picks up from the agent artifact), push the branch to TFS, look up the repo + project id, POST the PR, POST the PR review-notes thread, and PATCH the work item with a single atomic update that adds an `ArtifactLink` relation to the PR and transitions the tag from `agent-in-progress` to `agent-pr-opened`. You do not see the result of these calls — if any of them fail, gh-aw's incomplete-report channel surfaces it as a GitHub issue to the workflow maintainers.
